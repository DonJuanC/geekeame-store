...
Proyecto Integrador 5 (Full Stack), construido siguiendo una consigna técnica de e-commerce completo (catálogo con búsqueda, carrito, checkout, panel admin, integración con un storage externo vía backend serverless).
...
|---|---|---|---|
| 1 | ¿Estructura funcional (por tipo de archivo) o feature-based (por dominio) para organizar el proyecto? | La funcional es más simple de navegar en las primeras etapas; la feature-based escala mejor pero agrega complejidad innecesaria para el tamaño del proyecto. Separar Context/Provider/Hook en archivos propios por dominio favorece la responsabilidad única. | Estructura funcional (`components/`, `contexts/`, `hooks/`, `services/`, `types/`, `utils/`) con 4 contexts separados, cada uno con su hook con guard (`throw new Error` si se usa fuera del Provider). |
| 2 | ¿Por qué agregar el id manualmente en el servicio, y para qué sirven los índices compuestos? | Firestore separa `doc.id` de `doc.data()` — hay que combinarlos al mapear. Los índices compuestos son obligatorios cuando una query combina `where` + `orderBy` sobre campos distintos. | Mapeo manual de id en cada `service`; índices versionados en `firestore.indexes.json` en vez de creados a mano en la consola. |
| 3 | ¿Qué ventaja tiene separar componente contenedor de presentacional, y usar `children` en vez de props con nombre? | Presentacional es más reutilizable y testeable (responsabilidad única); `children` es más parecido a HTML, más simple para pasar JSX arbitrario que una prop dedicada. | `LoadingState`/`ErrorState`/`EmptyState` reutilizables; `ProductsTable`/`OrdersTable` presentacionales vs. páginas admin como contenedores; `RequireAdmin({ children })`. |
| 4 | Firestore no tiene búsqueda de texto completo — ¿cómo resolver la búsqueda por substring del catálogo? | Hay que precomputar un campo de búsqueda en minúsculas al guardar cada producto y consultarlo con `array-contains` (Firestore no soporta `LIKE`). | `searchKeywords`: array de prefijos por palabra, sin tildes, precomputado al guardar el producto; consultado con `array-contains`. |
| 5 | ¿Cuándo conviene `useState`, cuándo Context, y cuándo sumar `useReducer`? | El criterio depende de dos preguntas independientes: ¿se comparte el estado entre componentes? y ¿la lógica de cambio es compleja? Estado compartido + lógica compleja implica Context + `useReducer`. | El carrito cumple ambas condiciones (se dispara desde `ProductCard`, `CartPage` y el header; tiene 4 transiciones relacionadas) → `CartContext` + `cartReducer`. |
| 6 | ¿Dónde vive realmente la seguridad de Firebase si la config del proyecto es pública? | No vive en las credenciales (`firebaseConfig` es pública a propósito) sino en `firestore.rules` y la autenticación — por eso no hace falta un backend propio solo para ocultarla. | `AuthState` con `"loading"` como estado explícito (no solo `user` truthy/falsy); los guards (`RequireAuth`/`RequireAdmin`) verifican `loading` → `authenticated` → rol, en ese orden; el rol se hace cumplir en `firestore.rules`, no en el guard. |
| 7 | ¿Por qué no se puede subir una imagen directo a S3 desde React sin exponer las credenciales de AWS? | Cualquier credencial de AWS en el bundle del cliente queda visible en DevTools. El patrón correcto es que un backend mínimo firme una URL temporal (presigned URL) y el archivo viaje directo del navegador a S3. | `api/presign-upload.ts` (Vercel Function): valida rol admin reenviando el ID token de Firebase a la API REST de Firestore, firma la URL con el SDK de AWS v3 (60s, `ContentLength` fijado). |
| 8 | ¿Cómo evitar que dos compradores agoten el mismo stock al mismo tiempo? | Un `updateDoc` simple tiene condición de carrera real bajo concurrencia — hace falta leer, validar y escribir como una sola operación atómica. | `createOrder` usa `runTransaction`: lee el stock de todos los productos del carrito, valida, descuenta y crea la orden en una sola transacción; con un `withTimeout` de 20s si se cuelga. |
| 9 | ¿Qué hace falta testear (reducer, hooks, componentes, servicios) y qué necesita Vercel para que el ruteo de React Router sobreviva un refresh? | El reducer se testea puro sin renderizar nada; los servicios se mockean con `vi.mock` para no depender de Firestore real; Vercel no resuelve paths internos de una SPA sin un rewrite explícito. | `renderWithProviders` para tests de componentes; `vercel.json` con rewrite catch-all a `index.html`; variables sensibles de AWS nunca con prefijo `VITE_`. |
| 10 | ¿Por qué usar `useReducer` para el carrito en vez de `useState` o una librería como Zustand? | El carrito se dispara desde componentes muy distintos (`ProductCard`, `CartPage`, header) con operaciones acotadas (agregar/quitar/cambiar cantidad/vaciar) — ese patrón es el caso de uso típico de `useReducer`: centraliza la lógica de transición en una función pura, testeable en aislado, sin el boilerplate de una dependencia externa para un solo slice de estado. | `CartContext` + `useReducer`, sin librerías externas de manejo de estado. |
| 11 | ¿Cómo subir imágenes de producto a S3 sin exponer las credenciales de AWS en el frontend? | El patrón correcto es una presigned URL: un backend mínimo valida quién pide la subida y firma una URL de corta duración; el archivo va directo del navegador a S3, sin pasar por el backend ni por un servidor propio. | Vercel Function (`api/presign-upload.ts`) que valida rol admin reenviando el ID token a la API REST de Firestore, y devuelve una URL firmada con `expiresIn: 60`. |
| 12 | "Mis pedidos" tira error en producción pero funciona en local, ¿por qué? | La query combina `where(userId, ==, uid)` con `orderBy(createdAt)` — Firestore exige un índice compuesto para eso, y no se había creado en el proyecto de producción (sí existía, sin darse cuenta, en el de desarrollo). | Crear el índice desde el link que trae el error de Firestore en consola; documentar en el README que hay que recrearlo si el proyecto de Firebase cambia. |
| 13 | El botón "Todas" del catálogo y el logo del header deberían hacer lo mismo, ¿no? | No — son dos acciones distintas: "Todas" limpia el filtro de categoría pero se queda en la vista de catálogo; volver por el logo tiene que además "salir" del catálogo y mostrar la landing completa (hero, categorías, destacados). Necesitaban dos piezas de estado separadas en `ProductsContext`, no una sola bandera. | `goToLanding()` (limpia filtro/búsqueda y activa `showLanding`) separado del handler del pill "Todas" (solo limpia el filtro). |
| 14 | Después de un fix anterior, el corazón de favoritos se sale un poco del recuadro de la card — ¿qué cambió? | Se había agregado `relative` directo en el wrapper del botón, pero el caller ya le pasaba `absolute` por `className`. En el CSS que compila Tailwind, `relative` gana sobre `absolute` cuando ambas clases están presentes en el mismo elemento, sin importar el orden en que aparecen en el string — es el orden de las utilities en la hoja generada, no el de `className`, lo que decide. | Quitar el `relative` hardcodeado del wrapper: el componente confía en que el caller siempre pasa el positioning completo (`absolute top-N right-N z-10`) por props. |
| 15 | Después de la auditoría UI/UX completa, ¿cómo saber qué le falta al proyecto frente a lo que la rúbrica de evaluación realmente exige? | Comparar código contra una rúbrica externa con subagentes en paralelo (uno por bloque de criterios) encontró huecos que revisando pantalla por pantalla no aparecían — el más importante, que la bitácora de uso de IA no existía en el README, pesa en la nota tanto o más que varias mejoras de código juntas. | Priorizar el trabajo restante por impacto en la rúbrica (bitácora de IA primero, después `strict` mode, después el resto), no por lo que se nota más a simple vista. |
| 16 | Se pidió mostrar productos de varias categorías en "Destacados"; el primer fix se deployó pero el usuario seguía viendo lo mismo en producción — ¿por qué? | El primer intento elegía "hasta 1 producto por categoría" pero sobre la misma página ya paginada de productos recientes (`PRODUCTS_PAGE_SIZE`). Si esa ventana de 12 productos no incluía ninguna categoría (porque las últimas cargas del catálogo fueron todas de otra), el algoritmo de selección no tenía nada que elegir de ahí, sin importar qué tan bien estuviera escrito. Confirmarlo por lectura de código no alcanzaba — hizo falta navegar a producción con Claude in Chrome y capturar el estado real (1 categoría con 5 productos, el resto ausente) antes de descartar causas alternativas. | Reemplazar la fuente de datos, no el algoritmo: `listFeaturedCandidates()` consulta cada categoría por separado en Firestore (independiente de la paginación general), e `interleaveByCategory()` combina esos grupos ya diversos alternando ronda por ronda. |
